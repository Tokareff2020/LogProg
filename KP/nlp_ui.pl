...
ask_question(R, Ans) :-
    ...
    analis_question(Model, A, []),
    analis_model(Model, Args),
    generate_ans(Args, Ans).

read_string_to_list(S, L) :-
    split_string(S, " ", " ?", L).

analis_question(X) --> n1r_q(X). % Who is N R
analis_question(X) --> n2r_q(X). % Whose R is N  
analis_question(X) --> rn1n2_q(X). % What kind/type relations between N1 and N2
analis_question(X) --> num_q(X). % How many R does/did/do N have
